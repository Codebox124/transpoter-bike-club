...
export function EventCard({ title, date, location = "Main Track", imageUrl = "/event-default.jpg", category = "Race" }: EventCardProps) {
  return (
      ...
      <div className="p-6 border-l-4 border-green-500">
        <div className="flex items-center mb-4">
          <div className="bg-gray-900 rounded-md px-3 py-2 mr-4 text-center">
            <span className="block text-green-500 text-xs font-medium">{new Date(date).toLocaleString('default', { month: 'short' })}</span>
            <span className="block text-white text-xl font-bold">{new Date(date).getDate()}</span>
          </div>
